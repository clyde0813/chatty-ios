    ...
    var body: some View {
        GeometryReader{ proxy in
            ZStack(alignment: .bottom){
                ZStack(alignment: .bottom){
                    KFImage(URL(string: "\(chatty.profile.backgroundImage)"))
                        .resizable()
                        .scaledToFill()
                        .padding(-30)
                        .blur(radius: 20)
                    VStack(spacing: 0){
                        ...
                        ZStack{
                            ...
                            VStack(alignment: .leading, spacing: 0){
                                if chatty.author == nil {
                                    HStack(spacing: 0){
                                        Text("To ")
                                            .font(Font.system(size: 12, weight: .semibold))
                                        Text("@\(chatty.profile.username)")
                                    ...
                                    HStack{
                                        KFImage(URL(string:chatty.author?.profileImage ?? ""))
                                                .resizable()
                                                .scaledToFill()
                                        ...
                                        VStack(alignment: .leading, spacing: 0){
                                            HStack(spacing: 4){
                                                Text(chatty.author?.profileName ?? "")
                                                    .font(.system(size:12, weight: .bold))
                                                Text("@\(chatty.author?.username ?? "")")
                                                    .font(Font.system(size: 12, weight: .semibold))
                                                Spacer()
                                            }
                                            .padding(.bottom, 4)
                                ...
                                HStack(alignment: .top, spacing: 0){
                                    ...
                                    Image(systemName: "arrow.turn.down.right")
                                        ...
                                        .padding(.trailing, 12)
                                    //Profile 사진
                                    KFImage(URL(string: "\(chatty.profile.profileImage)"))
                                        .resizable()
                                        .scaledToFill()
                                    ...
                                    VStack(alignment: .leading, spacing: 0){
                                        HStack(spacing: 4){
                                            Text("\(chatty.profile.profileName )")
                                                .font(Font.system(size: 16, weight: .bold))
                                        }
                                        .padding(.bottom, 8)
                                        Text(chatty.answerContent ?? "")
